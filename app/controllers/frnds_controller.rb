...
class FrndsController < ApplicationController
  ...
  def create
   ...
   @frnd=correct_user.frnds.build(frnd_params)
  ...
  def correct_user
    @frnd=correct_user.frnds.find(id: params[:id]) 
  ...
  private
    ...
    def frnd_params
      params.require(:frnd).permit(:first_name, :l_name, :email, :phone, :twitter, :user_id)
    end
end
